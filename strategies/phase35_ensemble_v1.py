...
class Phase35EnsembleV1(BaseStrategy):
    ...
    def compute_signal(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"❌ [phase35_ensemble_v1] Signal computation exception: {e}")
            if self._diag_enabled:
    ...
    def _get_sub_model_votes(self, df: pd.DataFrame, regime: str) -> Dict[str, Dict[str, Any]]:
        """
        ...
        votes = {
            ...
            'reversion': self._sub_model_reversion(df, regime, sub_cfg.get('reversion', {})),
            'breakout': self._sub_model_breakout(df, regime, sub_cfg.get('breakout', {}))
        }
        
        return votes
    
    def _sub_model_trend(self, df: pd.DataFrame, regime: str, cfg: dict) -> Dict[str, Any]:
        """
        Sub-Model 1: Trend-Following (EMA Cross + ADX)
        ...
        """
        if regime != 'TREND':
            return {'direction': None, 'confidence': 0.0, 'reasons': ['regime_not_trend']}
    ...
    def _sub_model_reversion(self, df: pd.DataFrame, regime: str, cfg: dict) -> Dict[str, Any]:
        """
        Sub-Model 2: Mean-Reversion (RSI + Bollinger Bands)
    ...
    def _sub_model_breakout(self, df: pd.DataFrame, regime: str, cfg: dict) -> Dict[str, Any]:
        """
        Sub-Model 3: Breakout (ATR + Volume)
    ...
    def _calculate_bollinger(self, df: pd.DataFrame, period: int = 20, std: float = 2.0) -> Dict[str, pd.Series]:
