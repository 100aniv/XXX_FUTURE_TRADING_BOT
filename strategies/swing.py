...
def signal_logic(df: pd.DataFrame, config: dict) -> Dict[str, Any]:
    ...
    # BB/Donchian 돌파 (Donchian 컬럼 존재 시 OR)
    dc_has = ('dc_upper' in df.columns and 'dc_lower' in df.columns)
    donchian_up = dc_has and (last["close"] > last.get("dc_upper", last["bb_upper"]))
    donchian_down = dc_has and (last["close"] < last.get("dc_lower", last["bb_lower"]))
    breakout_long = allow_breakout and ((last["close"] > last["bb_upper"]) or donchian_up) and macd_up
    breakdown_short = allow_breakout and ((last["close"] < last["bb_lower"]) or donchian_down) and macd_down
