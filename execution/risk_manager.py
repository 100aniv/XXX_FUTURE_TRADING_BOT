...
class RiskManager:
    ...
    def save_state(self, db_conn, mode: str, run_id: str):
        """
        ⭐ PHASE7-2 항목 8: Risk Manager 상태 저장 (DB)
        ...
        try:
            # cooldown_until 계산
            cooldown_until = None
            if self.in_cooldown and self.cooldown_start_time > 0:
                from datetime import datetime, timedelta
                cooldown_until = datetime.fromtimestamp(self.cooldown_start_time) + timedelta(minutes=self.cooldown_minutes)
            
            with db_conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO trading.risk_state 
                    (mode, run_id, peak_equity, current_drawdown, consecutive_losses, in_cooldown, cooldown_until, updated_at)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, NOW())
                """, (mode, run_id, self.peak_equity, self.current_drawdown, self.consecutive_losses, self.in_cooldown, cooldown_until))
                db_conn.commit()
                logger.debug(
                    f"💾 [Risk] 상태 저장: peak=${self.peak_equity:,.0f}, "
                    f"consecutive_losses={self.consecutive_losses}, cooldown={self.in_cooldown}"
                )
        except Exception as e:
            logger.error(f"❌ [Risk] 상태 저장 실패: {e}")
            db_conn.rollback()
    
    def restore_state(self, db_conn, mode: str, run_id: str) -> bool:
        """
        ⭐ PHASE7-2 항목 8: Risk Manager 상태 복원 (DB 최신 행)
        ...
        try:
            with db_conn.cursor() as cur:
                cur.execute("""
                    SELECT peak_equity, current_drawdown, consecutive_losses, in_cooldown, cooldown_until, updated_at
                ...
                if row:
                    ...
                    self.current_drawdown = float(row[1])
                    self.consecutive_losses = int(row[2])
                    self.in_cooldown = bool(row[3])
                    cooldown_until = row[4]
                    updated_at = row[5]
                    
                    # cooldown_start_time 복원
                    if self.in_cooldown and cooldown_until:
                        from datetime import datetime
                        now = datetime.now()
                        if cooldown_until > now:
                            # 아직 쿨다운 중
                            remaining_minutes = (cooldown_until - now).total_seconds() / 60
                            self.cooldown_start_time = time.time() - (self.cooldown_minutes - remaining_minutes) * 60
                        else:
                            # 쿨다운 종료됨
                    ...
                    logger.info(
                        f"✅ [Risk] 상태 복원: peak=${self.peak_equity:,.0f}, "
                        f"consecutive_losses={self.consecutive_losses}, cooldown={self.in_cooldown} "
                        f"(마지막 업데이트: {updated_at})"
                    )
                    return True
                else:
                    logger.warning(f"⚠️ [Risk] 복원할 상태 없음 (mode={mode}, run_id={run_id})")
                    return False
        except Exception as e:
            logger.error(f"❌ [Risk] 상태 복원 실패: {e}")
            return False
