...
# -*- coding: utf-8 -*-
"""
Swing Bayesian Tuner (Optuna)
================================
- 목적: Swing 전략 파라미터 베이지안 최적화 (IS/OOS, WFA 지원)
- 실행: python -u scripts/tune_swing.py --study swing_v1 --trials 5 --use-wfa 1
...
def _build_overlay_from_trial(trial: 'optuna.trial.Trial') -> Dict[str, Any]:
    ...
    overlay = deep_merge(overlay, {'strategy': {'selector': 'swing'}})

    # Timeframe & Data: BACKTEST_PERIODS.md 준수 (Swing = 1h)
    ...
    if session_choice is not None:
        overlay = deep_merge(overlay, {
            ...
            'strategies': {'swing': {'filters': {'session_whitelist': session_choice}}}
        })
    else:
        ...
        overlay = deep_merge(overlay, {
            ...
            'strategies': {'swing': {'filters': {'session_whitelist': []}}}
        })
    ...
    # MTF/HTF/Trend align (완화: trend_align False 우선)
    overlay = deep_merge(overlay, {
    ...
    # Swing 전략 핵심 파라미터 (범위 확대)
    overlay = deep_merge(overlay, {
        'strategies': {'swing': {
            'rsi_long_min': float(trial.suggest_float('rsi_long_min', 25.0, 50.0)),  # 완화: 30~45 → 25~50
            'rsi_short_max': float(trial.suggest_float('rsi_short_max', 50.0, 75.0)),  # 완화: 55~70 → 50~75
            'ema_strict': trial.suggest_categorical('ema_strict', [False, True]),  # False 우선
            'allow_breakout': trial.suggest_categorical('allow_breakout', [True, False])
        }}
    })
    ...
    # Swing은 BB bounce 파라미터를 사용하지 않음 (전략 코드 기준)
    ...
    overlay = deep_merge(overlay, {
        'strategies': {'swing': {'filters': {'allow_short': allow_short}}}
    })
...
def objective_builder(base_cfg_path: Path, study_dir: Path, configs_dir: Path, study_name: str, use_wfa: bool, telegram_cfg: Dict[str, Any]):
    ...
    def objective(trial: 'optuna.trial.Trial') -> float:
        ...
        for key in [
            ...
            'rsi_long_min','rsi_short_max','ema_strict','allow_breakout',
            'min_rr_required','trail_k','be_at_r','allow_short','vol_spike_mult']:
...
def save_best_config(study: 'optuna.Study', base_cfg_path: Path, study_dir: Path) -> Path:
    ...
    out_path = project_root / 'configs' / f'best_swing_{study.study_name}.yml'
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def main():
    print("=" * 80, flush=True)
    print("🚀 SWING TUNER 시작", flush=True)
    print("=" * 80, flush=True)
    
    ap = argparse.ArgumentParser()
    ap.add_argument('--study', default='swing_v1')
    ...
    configs_dir = project_root / 'configs' / 'swing' / study_name
    ...
    try:
        if args.notify_progress and telegram_cfg.get('enable_telegram') and telegram_cfg.get('telegram_token') and telegram_cfg.get('telegram_chat_id'):
            ...
            system_name = telegram_cfg.get('system_name', 'SWING_TUNER')
    ...
    try:
        if args.apply_best:
            dest = project_root / 'configs' / 'best_swing.yml'
    ...
    try:
        if args.notify_completion and telegram_cfg.get('enable_telegram') and telegram_cfg.get('telegram_token') and telegram_cfg.get('telegram_chat_id'):
            ...
            if args.apply_best:
                msg += " | applied=configs/best_swing.yml"
            tg(msg, telegram_cfg)
    except Exception:
        pass
