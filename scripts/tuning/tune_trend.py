...
# -*- coding: utf-8 -*-
"""
Trend Bayesian Tuner (Optuna)
================================
- 목적: 트렌드 전략 파라미터 베이지안 최적화 (IS/OOS, WFA 지원)
- 실행: python -u scripts/tune_trend.py --study trend_v1 --trials 5 --use-wfa 1
...
def _build_overlay_from_trial(trial: 'optuna.trial.Trial') -> Dict[str, Any]:
    """Trend 전략용 Overlay 구성 (전략 파라미터 중심)"""
    overlay: Dict[str, Any] = {}
    overlay = deep_merge(overlay, {'strategy': {'selector': 'trend'}})

    # Timeframe & Data: BACKTEST_PERIODS.md 준수 (Trend = 4h)
    overlay = deep_merge(overlay, {
        'timeframe': '4h',
        'lookback': 400,
        'backtest': {
            'data_file': 'BTCUSDT_4h_bear_2018.csv'  # WFA 자동 감지용 seed (4h 파일명 패턴)
        }
    })
    ...
    # 세션 필터 (4h는 비활성화 고정)
    session_alias = trial.suggest_categorical('session_whitelist_alias', ['none'])
    session_choice = None
    # Explicitly clear any base session whitelist
    overlay = deep_merge(overlay, {
        'session_whitelist': [],
        'filters': {'session_whitelist': []},
        'strategies': {'trend': {'filters': {'session_whitelist': []}}}
    })

    # 연속 손실 가드/쿨다운 (완화: cooldown 0부터 허용)
    overlay = deep_merge(overlay, {
    ...
    # Trend 전략 신호 파라미터 (범위 확대)
    rsi_long_min = float(trial.suggest_float('rsi_long_min', 30.0, 55.0))  # 완화: 35~50 → 30~55
    rsi_long_max = float(trial.suggest_float('rsi_long_max', 60.0, 80.0))  # 완화: 65~75 → 60~80
    rsi_short_max = float(trial.suggest_float('rsi_short_max', 45.0, 70.0))  # 완화: 50~65 → 45~70
    rsi_short_min = float(trial.suggest_float('rsi_short_min', 20.0, 40.0))  # 완화: 25~35 → 20~40
...
def _scan_wfa_oos_files(base_cfg: Dict[str, Any]) -> List[Path]:
    """data_dir에서 OOS 파일 리스트 추출 (4h Trend전용)"""
    backtest = base_cfg.get('backtest', {})
    data_dir = Path(backtest.get('data_dir', 'data'))
    files: List[Path] = []

    # Trend는 4h timeframe만 사용
    # 패턴: BTCUSDT_4h_*WFA*OOS*.csv
    search_dirs = [data_dir, data_dir / 'wfa_blocks']
    
    for search_dir in search_dirs:
        if search_dir.exists():
            files.extend(search_dir.glob("BTCUSDT_4h_*WFA*OOS*.csv"))
...
def objective_builder(base_cfg_path: Path, study_dir: Path, configs_dir: Path, study_name: str, use_wfa: bool, telegram_cfg: Dict[str, Any]):
    ...
    def objective(trial: 'optuna.trial.Trial') -> float:
        ...
        for key in [
            ...
            'ema_strict', 'allow_short', 'min_rr_required', 'trail_k', 'be_at_r']:
...
def save_best_config(study: 'optuna.Study', base_cfg_path: Path, study_dir: Path) -> Path:
    ...
    out_path = project_root / 'configs' / f'best_trend_{study.study_name}.yml'
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def main():
    print("=" * 80, flush=True)
    print("🚀 TREND TUNER 시작", flush=True)
    print("=" * 80, flush=True)
    
    ap = argparse.ArgumentParser()
    ap.add_argument('--study', default='trend_v1')
    ...
    configs_dir = project_root / 'configs' / 'trend' / study_name
    ...
    try:
        if args.notify_progress and telegram_cfg.get('enable_telegram') and telegram_cfg.get('telegram_token') and telegram_cfg.get('telegram_chat_id'):
            ...
            system_name = telegram_cfg.get('system_name', 'TREND_TUNER')
    ...
    try:
        if args.apply_best:
            dest = project_root / 'configs' / 'best_trend.yml'
    ...
    try:
        if args.notify_completion and telegram_cfg.get('enable_telegram') and telegram_cfg.get('telegram_token') and telegram_cfg.get('telegram_chat_id'):
            ...
            if args.apply_best:
                msg += " | applied=configs/best_trend.yml"
            tg(msg, telegram_cfg)
    except Exception:
        pass
