...
# -*- coding: utf-8 -*-
"""
Breakout Bayesian Tuner (Optuna)
================================
- 목적: Breakout 전략 파라미터 베이지안 최적화 (IS/OOS, WFA 지원)
- 실행: python -u scripts/tune_breakout.py --study breakout_v1 --trials 5 --use-wfa 1
...
def _build_overlay_from_trial(trial: 'optuna.trial.Trial') -> Dict[str, Any]:
    ...
    overlay = deep_merge(overlay, {'strategy': {'selector': 'breakout'}})

    # Timeframe & Data: BACKTEST_PERIODS.md 준수 (Breakout = 1h)
    overlay = deep_merge(overlay, {
        'timeframe': '1h',
        'lookback': 400,
        'backtest': {
            'data_file': 'BTCUSDT_1h_bear_2018.csv'  # WFA 자동 감지용 seed (1h 파일명 패턴)
        }
    })
    ...
    if session_choice is not None:
        overlay = deep_merge(overlay, {
            ...
            'strategies': {'breakout': {'filters': {'session_whitelist': session_choice}}}
        })
    else:
        ...
        overlay = deep_merge(overlay, {
            ...
            'strategies': {'breakout': {'filters': {'session_whitelist': []}}}
        })

    # Cooldown & streak guard (완화: cooldown 0부터 허용)
    overlay = deep_merge(overlay, {
    ...
    # MTF/HTF/Trend align (완화: 필터 False 우선)
    overlay = deep_merge(overlay, {
        'enable_mtf_confirm': trial.suggest_categorical('enable_mtf_confirm', [False, True]),
        'htf': trial.suggest_categorical('htf', ['15m', '1h', '4h']),
        'filters': {'require_trend_align': trial.suggest_categorical('require_trend_align', [False, True])}
    })

    # Disable global gates that can silently reject signals
    overlay = deep_merge(overlay, {'filters': {'require_trend_align': False}})
    overlay = deep_merge(overlay, {
        'strategies': {'breakout': {'filters': {
            'regime': False,
            'volume_spike': False,
            'mtf_confirm': False
        }}}
    ...
    overlay = deep_merge(overlay, {
        'strategies': {'breakout': {'volume_mult': float(trial.suggest_float('scalp_volume_mult', 1.2, 2.0))}}
    })

    # Optional RSI filter for breakout (완화: RSI 필터 OFF 고정)
    overlay = deep_merge(overlay, {
        'strategies': {'breakout': {
            'rsi_min': float(trial.suggest_float('rsi_min', 20.0, 50.0)),  # 완화: 25~50 → 20~50
            'rsi_max': float(trial.suggest_float('rsi_max', 50.0, 85.0)),  # 완화: 50~80 → 50~85
            'filters': {'enable_rsi_filter': trial.suggest_categorical('enable_rsi_filter', [False])}  # False 고정
        }}
    })

    # Entry RR gate (완화: 1.0부터 허용)
    overlay = deep_merge(overlay, {
        'entries': {'min_rr_required': float(trial.suggest_float('min_rr_required', 1.0, 1.6))}
    })

    # Breakout 전략은 BB bounce 파라미터를 사용하지 않음
    ...
    # Short 허용 옵션 추가 (진입 기회 2배)
    allow_short = trial.suggest_categorical('allow_short', [False, True])
    overlay = deep_merge(overlay, {
        'strategies': {'breakout': {'filters': {'allow_short': allow_short}}}
    })
...
def _scan_wfa_oos_files(base_cfg: Dict[str, Any]) -> List[Path]:
    """data_dir에서 OOS 파일 리스트 추출 (1h Breakout전용)"""
    backtest = base_cfg.get('backtest', {})
    data_dir = Path(backtest.get('data_dir', 'data'))
    files: List[Path] = []

    # Breakout은 1h timeframe만 사용
    # 패턴: BTCUSDT_1h_*WFA*OOS*.csv
    search_dirs = [data_dir, data_dir / 'wfa_blocks']
    
    for search_dir in search_dirs:
        if search_dir.exists():
            files.extend(search_dir.glob("BTCUSDT_1h_*WFA*OOS*.csv"))
...
def objective_builder(base_cfg_path: Path, study_dir: Path, configs_dir: Path, study_name: str, use_wfa: bool, telegram_cfg: Dict[str, Any]):
    ...
    def objective(trial: 'optuna.trial.Trial') -> float:
        ...
        for key in [
            ...
            'enable_mtf_confirm','htf','require_trend_align',
            'vol_spike_mult','scalp_volume_mult',
            'rsi_min','rsi_max','enable_rsi_filter',
            'min_rr_required','trail_k','be_at_r','allow_short']:
...
def save_best_config(study: 'optuna.Study', base_cfg_path: Path, study_dir: Path) -> Path:
    ...
    out_path = project_root / 'configs' / f'best_breakout_{study.study_name}.yml'
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def main():
    print("=" * 80, flush=True)
    print("🚀 BREAKOUT TUNER 시작", flush=True)
    print("=" * 80, flush=True)
    
    ap = argparse.ArgumentParser()
    ap.add_argument('--study', default='breakout_v1')
    ...
    configs_dir = project_root / 'configs' / 'breakout' / study_name
    ...
    try:
        if args.notify_progress and telegram_cfg.get('enable_telegram') and telegram_cfg.get('telegram_token') and telegram_cfg.get('telegram_chat_id'):
            ...
            system_name = telegram_cfg.get('system_name', 'BREAKOUT_TUNER')
    ...
    try:
        if args.apply_best:
            dest = project_root / 'configs' / 'best_breakout.yml'
    ...
    try:
        if args.notify_completion and telegram_cfg.get('enable_telegram') and telegram_cfg.get('telegram_token') and telegram_cfg.get('telegram_chat_id'):
            ...
            if args.apply_best:
                msg += " | applied=configs/best_breakout.yml"
            tg(msg, telegram_cfg)
    except Exception:
        pass
