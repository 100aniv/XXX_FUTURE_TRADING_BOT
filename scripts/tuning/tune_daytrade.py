...
# -*- coding: utf-8 -*-
"""
Daytrade Bayesian Tuner (Optuna)
================================
- 목적: Daytrade 전략 파라미터 베이지안 최적화 (IS/OOS, WFA 지원)
- 실행: python -u scripts/tune_daytrade.py --study daytrade_v1 --trials 5 --use-wfa 1
...
def _build_overlay_from_trial(trial: 'optuna.trial.Trial') -> Dict[str, Any]:
    """Daytrade 전략용 Overlay 구성"""
    overlay: Dict[str, Any] = {}
    overlay = deep_merge(overlay, {'strategy': {'selector': 'daytrade'}})

    # Timeframe & Data: BACKTEST_PERIODS.md 준수 (Daytrade = 15m)
    overlay = deep_merge(overlay, {
        ...
        'strategies': {'daytrade': {'timeframe': '15m'}}
    })
    ...
    if session_choice is not None:
        overlay = deep_merge(overlay, {
            ...
            'strategies': {'daytrade': {'filters': {'session_whitelist': session_choice}}}
        })
    else:
        ...
        overlay = deep_merge(overlay, {
            ...
            'strategies': {'daytrade': {'filters': {'session_whitelist': []}}}
        })

    # Guards (완화: cooldown 0부터 허용)
    overlay = deep_merge(overlay, {
    ...
    # Entry/Exit common (완화: min_rr 1.0부터 허용)
    overlay = deep_merge(overlay, {
    ...
    overlay = deep_merge(overlay, {
        'strategies': {'daytrade': {'filters': {'allow_short': allow_short}}}
    })
...
def save_best_config(study: 'optuna.Study', base_cfg_path: Path, study_dir: Path) -> Path:
    ...
    out_path = project_root / 'configs' / f'best_daytrade_{study.study_name}.yml'
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def main():
    print("=" * 80, flush=True)
    print("🚀 DAYTRADE TUNER 시작", flush=True)
    print("=" * 80, flush=True)
    
    ap = argparse.ArgumentParser()
    ap.add_argument('--study', default='daytrade_v1')
    ...
    configs_dir = project_root / 'configs' / 'daytrade' / study_name
    ...
    try:
        if args.notify_progress and telegram_cfg.get('enable_telegram') and telegram_cfg.get('telegram_token') and telegram_cfg.get('telegram_chat_id'):
            ...
            system_name = telegram_cfg.get('system_name', 'DAYTRADE_TUNER')
    ...
    try:
        if args.apply_best:
            dest = project_root / 'configs' / 'best_daytrade.yml'
    ...
    try:
        if args.notify_completion and telegram_cfg.get('enable_telegram') and telegram_cfg.get('telegram_token') and telegram_cfg.get('telegram_chat_id'):
            ...
            if args.apply_best:
                msg += " | applied=configs/best_daytrade.yml"
            tg(msg, telegram_cfg)
    except Exception:
        pass
