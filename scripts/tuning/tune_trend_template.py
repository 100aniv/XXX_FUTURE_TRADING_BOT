...
def _build_overlay_from_trial(trial: 'optuna.trial.Trial') -> Dict[str, Any]:
    ...
    if session_choice is not None:
        overlay = deep_merge(overlay, {
            ...
            'strategies': {'trend': {'filters': {'session_whitelist': session_choice}}}
        })
    ...
    # Trend 전략 신호 파라미터
    rsi_long_min = float(trial.suggest_float('rsi_long_min', 35.0, 50.0))
    rsi_long_max = float(trial.suggest_float('rsi_long_max', 65.0, 75.0))
    rsi_short_max = float(trial.suggest_float('rsi_short_max', 50.0, 65.0))
    rsi_short_min = float(trial.suggest_float('rsi_short_min', 25.0, 35.0))
    ema_strict = trial.suggest_categorical('ema_strict', [True, False])
...
def objective_builder(base_cfg_path: Path, study_dir: Path, configs_dir: Path, study_name: str, use_wfa: bool, telegram_cfg: Dict[str, Any]):
    ...
    def objective(trial: 'optuna.trial.Trial') -> float:
        ...
        print(f"\n  🔧 테스트 파라미터:", flush=True)
        param_items = []
        for key in ['ema_fast', 'ema_slow', 'atr_mult_sl', 'rr_ratio', 'allow_short']:
