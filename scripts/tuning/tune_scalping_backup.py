...
def objective_builder(base_cfg_path: Path, study_dir: Path, configs_dir: Path, study_name: str, use_wfa: bool, telegram_cfg: Dict[str, Any]):
    ...
    def objective(trial: 'optuna.trial.Trial') -> float:
        ...
        print(f"\n  🔧 테스트 파라미터:", flush=True)
        param_items = []
        for key in ['rsi_min', 'rsi_max', 'cooldown_candles', 'atr_mult_sl', 'rr_ratio', 'allow_short']:
        ...
        for idx, data_file in enumerate(eval_files):
            ...
            print(f"  📄 [{idx+1}/{len(eval_files)}] {filename}: score={seg_score:.2f}, trades={t}", flush=True)
        ...
        # 🎯 최종 집계 점수 출력
        print(f"\n  🎯 최종 점수: {score:.2f} (avg of {len(scores)} segments, total trades={trades_total})", flush=True)
