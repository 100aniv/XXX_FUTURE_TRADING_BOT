...
# -*- coding: utf-8 -*-
"""
PHASE21-1C Rapid Test: 7 Strategies × 5min Each
================================================
...
def main():
    """Main execution"""
    safe_print("\n" + "="*70)
    safe_print("PHASE21-1C: Rapid Strategy Tests (5min Each)")
    safe_print("="*70)
    safe_print(f"Start: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    safe_print(f"Total estimated duration: ~{len(STRATEGIES) * TEST_DURATION_MIN}min")
    safe_print("="*70)
    ...
    results_file = docs_dir / "phase21_1c_rapid_results.json"
