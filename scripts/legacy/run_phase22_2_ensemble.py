...
# -*- coding: utf-8 -*-
"""
PHASE22-2: Ensemble v2 Extended Validation Runner (12~24H)
===========================================================
목표: 5개 전략 (scalping_v3 + 4개 v2) 통합 장기 안정성 검증
Runtime: 12~24H wall-clock Paper 테스트
Symbol: BTCUSDT (단일 심볼)

Usage:
    python scripts/run_phase22_2_ensemble.py
    python scripts/run_phase22_2_ensemble.py --config configs/paper/phase22_2_ensemble_quick.yml
    python scripts/run_phase22_2_ensemble.py --config configs/paper/phase22_2_ensemble_12h.yml --duration-hours 12

Output:
    scorecards/paper_phase22_2/{run_id}/
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='PHASE22-2: Ensemble v2 Extended Validation (12~24H)',
    ...
    parser.add_argument(
        '--config',
        type=str,
        default='configs/paper/phase22_2_ensemble_quick.yml',
        help='Config 파일 경로 (기본: configs/paper/phase22_2_ensemble_quick.yml)'
    )
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("🚀 PHASE22-2: Ensemble v2 Extended Validation")
    logger.info("=" * 80)
    ...
    # Output directory 생성
    output_dir = Path("scorecards") / "paper_phase22_2" / run_id
    ...
    # 5. 엔진 실행
    logger.info("=" * 80)
    logger.info("🟢 Paper Trading 시작 (PHASE22-2 Extended Validation)")
    logger.info("=" * 80)
    ...
    period_info = {
        ...
        'mode': 'PHASE22-2_ENSEMBLE_V2'
    }
    
    try:
        generator = ScorecardGenerator(
            strategy_name='ensemble_v2',
            symbol=symbol,
