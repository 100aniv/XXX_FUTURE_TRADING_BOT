...
- [x] 튜닝 파라미터/오버레이 구조 설계 완료(PR13 연계) ✅
- [x] A/B 비교 리포트(페이퍼) 산출 경로 정의 ✅
- [ ] 24시간 페이퍼 평가 (baseline 대비 성능 비교) - 진행 중 (2025-11-07 13:00 완료 예정)
...
- **2025-11-06 13:50** | 🐛 Bug #2: pnl_pct 미계산 | trading.trades 테이블의 pnl_pct 컬럼이 NULL | engine.py에서 pnl_pct 계산하지만 DB 저장 로직 누락 | 추후 수정 필요 (백분율 분석 영향 제한적)
- **2025-11-06 13:50** | 🐛 Bug #3: 자산 초과 경고 | "총 자산 가치 초과: $10000.49 > $10000.00" 반복 경고 | 포지션 관리 로직의 소수점 오차 | 추후 수정 필요 (기능 영향 없음)
