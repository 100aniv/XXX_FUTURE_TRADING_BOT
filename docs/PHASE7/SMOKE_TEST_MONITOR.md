...
- **역할**: PHASE7-1 변경 사항(수수료, OHLC SL, Extreme Loss) 적용 상태를 실측 지표로 검증하는 모니터 문서.
- **현행**: 안정 버전(b84c03c). 핵심 기능 정상, 슬리피지 미구현. 무결성/손실 상한/승률을 정기 스냅샷으로 트래킹.
- **방침**: 30/60m 실시간 확인 + 2h/24h 스냅샷 고정. A/B는 run_id 격리로 1h 단위 비교.

## 🔎 Quick Nav

- [현재 상태 스냅샷(30/60m)](#-현재-상태-스냅샷-최근-3060분--paper)
- [실측 스냅샷 결과 (2시간)](#-실측-스냅샷-결과-최근-2시간)
- [실측 스냅샷 결과 (24시간)](#-실측-스냅샷-결과-최근-24시간)
- [사용한 SQL](#-사용한-sql-쿼리)
- [A/B 1시간 대조 절차](#-ab-1시간-대조-실험-절차)
- [시작 시간/환경](#-시작-시간)
