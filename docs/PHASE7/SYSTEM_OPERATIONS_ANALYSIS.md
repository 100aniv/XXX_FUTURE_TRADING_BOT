...
- **역할**: 운영 안정성 표준(종료/재시작/복원/Docker/모니터링). Live 전환 게이트의 기반 문서.
- **현행**: 안정 버전(b84c03c) 기준. SL 거래소 등록만 사용, TP는 메모리 관리. 상태 DB 미구현.
- **방침**: 비정상 종료에도 SL 보장, 재시작 직후 상태 완전 복원, Paper/Live 동일 순서 유지.
