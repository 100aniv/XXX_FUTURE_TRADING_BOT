...
- **역할**: MASTER BASELINE. PHASE7 전체 문서의 기준이 되는 TO-BE 설계서.
- **현행**: 안정 버전(b84c03c) 기준. 슬리피지 미구현, 전략별 독립 설정 미구현.
- **방침**: PHASE7-2 재개 시 전략별 제한·거래 빈도 제한·가중치 강화부터 적용. 슬리피지는 재설계 후.
- **스냅샷**: 최근 2h/24h 성과는 아래 Standard Snapshot 및 SMOKE_TEST_MONITOR.md 참조.

## 🔎 Quick Nav

- [현재 시스템 구조](#현재-시스템-구조-완전-이해)
- [상용 프로그램 벤치마킹](#상용-프로그램-벤치마킹)
- [핵심 문제](#핵심-문제-분석)
- [개선안](#앙상블-특화-개선안)
- [PHASE별 적용](#phase별-적용-계획)
